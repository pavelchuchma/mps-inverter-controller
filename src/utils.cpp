...
static void rotateAppLog() {
  ...
  while (copied) {
    ...
    if (n <= 0) break;
    if (dst.write(buf, (size_t)n) != (size_t)n) copied = false;  // out of space
  }
  src.close();
  dst.close();

  if (!copied) {
    // Rotation ran out of room. Keep the original: an oversized log still holds
    // its history, whereas replacing it with a truncated copy would destroy the
    // very thing rotation exists to preserve.
    LittleFS.remove("/app.log.tmp");
    return;
  }
